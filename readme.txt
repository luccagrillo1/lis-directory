...
* Someone already paying for Standard who buys Featured pays for both; cancel the old Standard subscription in WooCommerce by hand.
* Fixed the private claim-link page's menu bar being invisible (white text on a white page): it inherited the home page's transparent header. It now uses the normal solid header.
...
* Dashboard now matches Directorist's reference layout more closely: All Listings/Published/Pending/Draft/Expired filter tabs, plus Expiration Date and Plan columns.
* Removed the temporary diagnostic from 0.36.31.
...
* Still carrying the temporary gettext diagnostic from 0.36.26 (?lis_debug_gettext=1) while the Jetpack Comments "Leave a Reply" relabel is tracked down.


= 0.36.26 =
* Temporary diagnostic build to track down why the Jetpack Comments "Leave a Reply" text wasn't relabeling (gated behind ?lis_debug_gettext=1, no visible change).
...
= 0.36.20 =
* Vendor Showcase heading now shows a small "i" info icon with a "Sponsored" tooltip.
* The heading's lockup image links to a new "Vendor Showcase" browse-all page (auto-created), listing every active vendor as a card grid via the new [lis_preferred_vendor_directory] shortcode.


= 0.36.19 =
* Fixed the single-listing hero photo cropping portrait photos badly (e.g. a headshot losing the top of the head and chin). It now shows the whole photo (object-fit: contain), letterboxed on a neutral fill when the photo is taller than the hero cap, instead of zoom-cropping.
