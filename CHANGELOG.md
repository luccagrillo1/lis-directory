...
- **`lis_directory_maybe_unpublish_unpaid_listing()`** (`includes/listings-pricing.php`): drafts a *currently published* listing if `lis_directory_listing_has_any_active_tier()` — Standard active, Featured, or an active Vendor Showcase — is false. Called at the end of all three subscription-ended paths: `lis_directory_handle_featured_listing_subscription_ended()`, `lis_directory_handle_standard_listing_subscription_ended()`, and `lis_directory_sync_featured_with_showcase()` (Showcase's own ending, reached via `lis_directory_handle_subscription_ended()` in `includes/woocommerce.php` → `lis_directory_set_vendor_status()` → the `lis_directory_vendor_status_changed` hook). Re-derives the *current* state after that handler's own flag updates, so a listing kept alive by a different still-active tier is correctly left published.
- **Found and fixed while testing this:** `lis_directory_handle_standard_listing_subscription_ended()` still unconditionally cleared `_lis_listing_featured` and expired the linked Showcase vendor — a cascade left over from before Featured/Showcase became flat, self-contained tiers that *grant* Standard rather than requiring it as a bundled line. Under the current model that cascade ran backwards: cancelling an unrelated standalone Standard subscription would wrongly cancel a separate, still-paid Featured or Showcase on the same listing. It now only clears Standard's own flag, and only when nothing else (Featured, active Showcase) is currently granting it.
- **Not done:** no retroactive sweep of already-published listings whose subscription already ended before this shipped — this only applies going forward, from the next subscription-ended event.

Verified in the local WordPress harness (real WooCommerce 11.1, subscription-ended handlers called directly since WooCommerce Subscriptions itself isn't installable there): Standard-only listing drafts when its subscription ends; Featured-only drafts (and Standard, which Featured had granted, is revoked too); a Showcase-only listing (Jaden DuMars's real case: order #8669/subscription #8670) drafts when the vendor is expired; a listing with an independent Standard subscription that ends while its separate Showcase stays active correctly remains published and keeps Standard true; an admin-created listing with no subscription is untouched; calling the check on an already-draft listing is a no-op.
...
- **False "taken" fix.** Reported live: the claim picker said an Insurance listing's slot was taken though nobody held Insurance. Two causes found in the code, either of which produces it: (1) `lis_directory_get_active_vendor_for_category()` used WP's default `include_children => true`, so an active vendor under any CHILD category counted as holding the parent; now exact-category. (2) The picker/handler judged a listing by `get_the_terms()[0]`, arbitrary when a listing has several categories (e.g. a broad parent + "Insurance Agencies"); now `lis_directory_get_listing_showcase_term()` uses the most specific one (also used by the bundle-checkout taken check). I could not inspect the live data, so which of these (or a genuinely stale active vendor) applies there is unconfirmed — the new page shows the holder, and the picker message now names the category and holder.
- **Picker copy:** taken → "The <Category> showcase slot is currently held by <Vendor>."; the listing's own active showcase → "Already active on this listing." (disabled either way).
- **Features box** (`includes/listings-features.php`): checkbox list of every feature on the listing edit screen + an add-new field (admin-created features are live, not pending); pending suggestions are flagged. WordPress's default tag box is removed (`meta_box_cb => false`) and the block editor's own panel hidden (REST `visibility.show_ui`) so there's a single writer. Saves only when the box's nonce is present, so front-end saves, quick edit and bulk edit are untouched; only real terms of the taxonomy are attached.

Verified in the local WordPress harness: features box render/save/clear/no-nonce/no-capability, default box removal, REST panel hiding; a parent/child category tree proving the parent is no longer "taken" by a child's vendor, the most-specific-category choice, holder lookup, all Slots page sections, Release with nonce/capability enforcement; and the three Showcase card states in the claim picker.
...
- **The $60 line:** *inferred, not confirmed.* It isn't one of this plugin's three tier products (it carried no `lis_listing_id` tag, and it can't be the Standard product itself — a second Standard line would have tripped the existing one-subscription-per-cart guard), and it isn't in the public product list. Most likely a leftover Directorist plan product in the buyer's persisted WooCommerce cart, but the site's cart/product data was never inspected. `lis_directory_purge_foreign_plan_cart_items()` now removes cart lines whose product (or parent) is Directorist's `listing_pricing_plans` type or is named "… - Local Business", on both checkout entry points (the bundle handler and a Standard-only `?add-to-cart=` via the `woocommerce_add_to_cart` hook), and tells the buyer. Nothing else in the cart is touched (extendable via the `lis_directory_is_foreign_plan_cart_item` filter). **If a $60 line still appears, its product is neither of those — tell me its product type/ID.**
...
- By request: "more listings per page just put them all on one page, endless scroll vibe." Two separate query paths fed listings pages, so both needed fixing:
  - The main `lis_listing` archive and `lis_listing_category` term archives (`templates/archive-listing.php`) use WordPress's global main query, which was falling back to Settings → Reading's "Blog pages show at most" with standard numbered pagination (`the_posts_pagination()`) — no plugin-side override existed at all. New `lis_directory_show_all_listings_on_one_page()` (includes/listings-search.php, hooked to `pre_get_posts` alongside the existing search-filter hook) sets `posts_per_page => -1` whenever the query targets either archive. `the_posts_pagination()` removed from the template (dead code now — a single page never has a second page to link to).
  - The `[lis_listing_grid]` shortcode (includes/listings-account.php) — which is what actually renders the site's real "All Listings" page (`/services/local-directory/local-business/`, confirmed live) — took its own separate `get_posts()` path with **no pagination UI at all**, just a hard `count` cap defaulting to 12; past the 12th listing was simply unreachable. Default changed from `12` to `-1` (unlimited), with `-1` explicitly exempted from the `max( 1, ... )` clamp so it doesn't get coerced to a 1-listing cap. A caller can still pass an explicit `count` for a genuine small preview widget.

## [0.36.39] — 2026-08-15 — Plan step redesign: checkboxes for Featured/Showcase

- By request: replace the "Choose your plan" single-select (Standard/Featured/Vendor Showcase as three mutually-exclusive radio cards) with a model matching how Standard actually works since v0.36.37 — it's a required foundation, not a competing choice. New heading "Want more eyes on your listing?"; Standard renders as its own locked/checked card tagged "Included" showing its own price; Featured and Vendor Showcase render as independent checkboxes (`name="lis_listing_upgrades[]"`), each showing its own incremental price (e.g. "+$19/mo") rather than a bundled total, since Standard is now visible on its own line. A new `.lis-listing-plan-total` line sums Standard + any checked upgrades live via JS as billing/checkboxes change, so nothing is a surprise at checkout.
- Backend generalized to support buying Featured AND Showcase together in the same checkout (previously the wizard only ever sent one tier). `lis_directory_get_listing_upgrade_url()` (single-tier, still used by the Dashboard's two separate upsell buttons) now delegates to a new plural `lis_directory_get_listing_upgrades_url( $listing_id, array $tiers, $billing )`; `lis_directory_handle_bundle_checkout()` parses `tier` as a comma-separated list and loops, applying the same per-product WC-Subscriptions cart-conflict check (from v0.36.38) to Standard and every requested upgrade. `lis_directory_build_listing_checkout_url()` now takes either the new upgrades array or the legacy single-tier string — the claim-listing flow (includes/listings-claim.php) still posts a single `lis_listing_plan` value and keeps working unchanged.
- `_lis_listing_pending_tier` now stores a comma-joined list (e.g. `standard,featured,showcase`) instead of one tier — the Dashboard's "Plan" column (includes/listings-account.php) joins the mapped labels with " + " for display. The wp-admin "Listing Details" meta box's single `<select>` for manually granting a tier is unchanged (lower-priority admin tool, not customer-facing) — it just won't pre-select anything for a multi-tier listing.

## [0.36.38] — 2026-08-15 — Fix silent cart conflict in Standard bundling

- Found while card-testing v0.36.37's bundle checkout on staging (after syncing production → staging via WordPress.com's built-in staging tools and confirming WooPayments/WC Subscriptions both auto-detect staging and run in test mode): a second test listing's Featured upgrade reached checkout missing its bundled Standard line. Root cause confirmed via a temporary debug log written directly through wp-admin's Plugin File Editor on staging (deployed as an isolated diagnostic, not a release) — checkout was showing WooCommerce Subscriptions' own notice, "You cannot add another '[Product]' to your cart", because a *different* listing's Standard/Featured line was already sitting in the same customer's cart (e.g. from an earlier, unfinished upgrade). WC Subscriptions rejects a second cart line for the same product outright; `WC()->cart->add_to_cart()` just returns `false` with a queued notice, no exception, and `lis_directory_handle_bundle_checkout()` never checked the result — so checkout proceeded anyway, silently short one line item.
- Fixed in `lis_directory_handle_bundle_checkout()` (includes/listings-pricing.php): before adding either product, a new `lis_directory_cart_has_product_for_other_listing( $product_id, $listing_id )` scans the cart for that same product tagged to a *different* `lis_listing_id`. On a conflict, the buyer is redirected to their cart with a clear notice ("Your cart already has '[Product]' for a different listing...") instead of continuing to a checkout that would be missing a line item.

## [0.36.37] — 2026-08-14 — Standard as a required foundation for Featured/Showcase

- By request: "everyone has to have a standard listing to be featured or in the vendor showcase." Implemented as automatic bundling rather than UI gating — less friction, and the codebase already tracked Featured (`_lis_listing_featured`) and Vendor Showcase (the linked vendor's own status) as independent flags with their own cancel-revoke logic, so bundling Standard in was a smaller, safer change than restructuring the wizard into a forced two-phase signup.
- New `lis_directory_get_listing_upgrade_url( $listing_id, $tier, $billing )` in includes/listings-pricing.php — shared by the Dashboard's upsell links and the initial wizard's Plan step. Returns a URL to a new `lis_directory_handle_bundle_checkout()` admin-post handler, which adds Standard to the cart (via `WC()->cart->add_to_cart()`) only when the listing doesn't already have `_lis_listing_standard_active`, then the Featured/Showcase product, then redirects to WooCommerce's normal checkout — the buyer sees both line items before paying, same as adding two products by hand.
- New meta: `_lis_listing_standard_active` (bool) / `_lis_listing_standard_order_id` (int), set on order completion the same way Featured already tracks itself. New `lis_directory_handle_standard_listing_subscription_ended()` cascades a Standard cancellation/expiration to clear `_lis_listing_featured` and expire the linked Vendor Showcase vendor (`lis_directory_set_vendor_status( $vendor_id, 'expired' )`) — Vendor Showcase's own subscription ending already does this independently (includes/woocommerce.php), so either one ending is now enough to take the slot down.
- New `lis_directory_get_showcase_upgrade_url()` and a "Join the Vendor Showcase" link on the Dashboard, alongside the existing "⭐ Feature this listing" one — previously there was no way to add Showcase to an already-published listing after the fact.
- `lis_directory_build_listing_checkout_url()` (the initial wizard) now delegates 'featured'/'showcase' to the same `lis_directory_get_listing_upgrade_url()`, so both entry points share one bundling implementation. Fixed a bug caught during review: the wizard's `$can_checkout` pre-validation calls this with `listing_id = 0` (before the post exists yet) — the new function initially required a truthy listing_id and would've silently broken the paid-checkout path for Featured/Showcase, falling back to the free-pending queue instead. Fixed by only gating on listing_id where a real post is actually needed.
- Plan step pricing (includes/listings-submission.php): Featured/Showcase now display the *real bundled total* (their own price + Standard's), not just their own upgrade price — a brand-new listing always needs Standard bundled in, so showing just the add-on price would have understated the actual checkout total.
...
- `.lis-listing-wizard-progress` thickened from 6px to 10px.
- "Save & finish later": previously a standalone bar above the wizard with its own hint text. Now a single real button re-parented by JS (`listing-form-wizard.js`) into whichever panel's new `.lis-listing-wizard-panel-section-row` is currently active, mirroring the section label on the same row instead of occupying its own. The redundant "Saves what you've got so far..." hint text was removed from listings-submission.php — it's already shown on the Draft Saved confirmation screen.
- `.lis-listing-thankyou-text` margin-bottom reduced 28px → 20px for a tighter, more consistent rhythm with the icon/title spacing above it.
- `.lis-listing-card-excerpt` was missing an explicit `line-height`, inheriting the theme's loose 1.7em body default — set to 1.5.
- `lis_directory_render_listing_card()` (includes/listings-account.php): Featured/Popular/Verified badges and the category badge moved from their own row in the card body onto the thumbnail as overlays (status badges top-left, category top-right), with a fallback to the old body-row placement when a listing has no photo. New `.lis-listing-card-thumb-wrap` / `.lis-listing-card-thumb-badges` CSS.
- Search results List view: `.lis-listing-grid--list .lis-listing-card` had `align-items: stretch` with the thumb at `height: auto`, so a portrait photo's own intrinsic aspect ratio dictated the row height — a tall photo blew the row out, stranding short text at the top with a wall of empty space below. Fixed with an explicit `height: 150px` on the row and `height: 100%; object-fit: cover;` on the thumb.
- Search results Map view: previously hid the results grid entirely and showed only the map. Restructured (templates/archive-listing.php markup order + assets/js/listing-view-toggle.js `setView()`) so the map renders above the results instead, with results staying visible underneath in the same compact List-view layout — map-on-top, cards-below, similar to LIS Events' full calendar view.
- Price filter (`$` / `$$` / `$$$` / `$$$$` / Any) removed from the `[lis_listing_search]` "More Filters" panel for now, per request — wrapped in an `if ( false )` guard in includes/listings-search.php rather than deleted, and `lis_directory_get_price_tiers()` / the `lis_price` query-filtering logic are untouched, so it's a one-line flip to bring back.
- Removed the "All Directories" `<select>` from `[lis_listing_search]` entirely, by request — the underlying `directory="…"` type-lock still works via a hidden field for pages that pass it, there's just no visible picker for it anywhere now. Removed the now-dead `$current_type` local in `lis_directory_render_search_form_shortcode()`.
- `[lis_listing_grid]` (`lis_directory_render_grid_shortcode()`) brought to real feature parity with the category archive template, by request — it was the reason "All Listings" (a `[lis_listing_grid type="local-business"]` page) looked and behaved differently from a real `lis_listing_category` term archive: no toolbar, no sort, no map. Now calls the same `lis_directory_render_listing_toolbar()`, applies `lis_sort` to its own `get_posts()` query, and renders the same `data-lis-listing-map-view` block — map view works because `lis_directory_render_listing_card()` now emits the same `data-address`/`data-title` attributes the archive template's own card markup already had.
- Not done in this pass, flagged as a larger follow-up: fully unifying "browse all listings" and "single category" into one template/URL structure (the user's stated ideal) would mean either turning listing `type` into a real taxonomy or building custom rewrite-rule-based archives for it — a bigger architectural change than the feature-parity fixes above, which get both surfaces showing/behaving identically without it.
...
- Removed the domain check; `lis_directory_relabel_jetpack_comment_form()` now matches on the exact original strings + `lis_listing` post type only, regardless of domain.
